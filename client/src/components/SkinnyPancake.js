...
//Function for Skinny Pancake page
function SkinnyPancake() {
  //Sets the center of the map
  const [center, setCenter] = useState([44.4772, -73.2197]);
  //Sets the marker of the map
  const [marker, setMarker] = useState([44.4772, -73.2197]);
  ...
  useEffect(() => {
    //fetches data from skinnypancake.json
    fetch("/api/skinnypancake")
      ...
      .then((skinnyData) => {
        ...
        setName(skinnyData.name);
        //pulls in the address data
        setAddress(skinnyData.address);
        //pulls in the phone data
        setPhone(skinnyData.phone);
        //pulls in the hours data
        setHours(skinnyData.hours);
        //pulls in the notes data
        setNotes(skinnyData.notes);
      });
  }, []);
  ...
  return (
    <div>
      ...
      <div className="map-text-wrapper">
        <div>
          {/* The map for Skinny Pancake page */}
          <MapContainer
            center={center}
            zoom={17}
...
//Exports Skinny Pancake function to be used in other components
